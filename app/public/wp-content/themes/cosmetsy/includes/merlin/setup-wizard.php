...
*************************************************************/
function cosmetsy_local_import_files() {
	return array(
		array(
...
add_filter( 'merlin_import_files', 'cosmetsy_local_import_files' );

/************************************************************
## Setup Wizard After Import
*************************************************************/
function cosmetsy_merlin_after_import_setup() {
	// Assign menus to their locations.
	$main_menu 	  = get_term_by( 'name', 'Menu 1', 'nav_menu' );
	$topleftmenu  = get_term_by( 'name', 'Top Left', 'nav_menu' );
	$toprightmenu = get_term_by( 'name', 'Switcher', 'nav_menu' );
    ...
    if ( class_exists( 'woocommerce' ) ) {
        update_option( 'woocommerce_shop_page_id', '108' );
        update_option( 'woocommerce_cart_page_id', '109' );
        update_option( 'woocommerce_checkout_page_id', '110' );
        update_option( 'woocommerce_myaccount_page_id', '111' );
    }


}
add_action( 'merlin_after_all_import', 'cosmetsy_merlin_after_import_setup' );
