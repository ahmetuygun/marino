...
if(get_theme_mod('cosmetsy_paginate_type') == 'loadmore' || cosmetsy_ft() == 'load-more'){
	function cosmetsy_load_more_button(){
		echo '<nav class="woocommerce-pagination klb-load-more">
                <ul class="page-numbers">
					<div class="button light">'.esc_html__('Load More','cosmetsy-core').'</div>
				</ul>
			  </nav>';
	}

	remove_action( 'woocommerce_after_shop_loop', 'woocommerce_pagination', 10 );
	add_action( 'woocommerce_after_shop_loop', 'cosmetsy_load_more_button', 10 );
}
...
if(get_theme_mod('cosmetsy_paginate_type') == 'infinite' || cosmetsy_ft() == 'infinite'){
	remove_action( 'woocommerce_after_shop_loop', 'woocommerce_pagination', 10 );
}
...
if(get_theme_mod('cosmetsy_ajax_on_shop',0) == '1'){
	add_action('woocommerce_before_main_content','cosmetsy_loadmore_parameters',40);
	function cosmetsy_loadmore_parameters() {
	...
	<script type="text/javascript">
		var loadmore = {
			...
			"no_more_products":"<?php echo esc_html_e('No More Products','cosmetsy-core'); ?>",
...
## Quick View Scripts
*************************************************/
if(get_theme_mod('cosmetsy_paginate_type') == 'loadmore' || get_theme_mod('cosmetsy_paginate_type') == 'infinite' || cosmetsy_ft() == 'load-more' || cosmetsy_ft() == 'infinite'){
	if(get_theme_mod('cosmetsy_ajax_on_shop',0) == '1'){
		function cosmetsy_load_more_scripts() {
			if(is_shop() || is_tax()){
				if(get_theme_mod('cosmetsy_paginate_type') == 'infinite' || cosmetsy_ft() == 'infinite'){
					wp_enqueue_script( 'cosmetsy-load-more',  plugins_url( 'js/infinite-scroll.js', __FILE__ ), true );
				} elseif(get_theme_mod('cosmetsy_paginate_type') == 'loadmore' || cosmetsy_ft() == 'load-more') {
					wp_enqueue_script( 'cosmetsy-load-more',  plugins_url( 'js/load_more.js', __FILE__ ), true );
				}
			}
		}
		...
		function cosmetsy_load_more_scripts() {
			if(is_shop() || is_tax()){
				...
				wp_localize_script( 'cosmetsy-load-more', 'loadmore', array(
					...
					'filter_cat' => isset($_GET['filter_cat']) ? esc_html($_GET['filter_cat']) : '',
					...
					'shop_view' => isset($_GET['shop_view']) ? esc_html($_GET['shop_view']) : '',
					'min_price' => isset($_GET['min_price']) ? esc_html($_GET['min_price']) : '',
					'max_price' => isset($_GET['max_price']) ? esc_html($_GET['max_price']) : '',
					'no_more_products' => esc_html__('No More Products','cosmetsy-core'),
					'is_search' => is_search() ? 'yes' : '',
				));
			}
		}
	}
	add_action( 'wp_enqueue_scripts', 'cosmetsy_load_more_scripts' );
}
...
add_action( 'wp_ajax_nopriv_load_more', 'cosmetsy_load_more_callback' );
add_action( 'wp_ajax_load_more', 'cosmetsy_load_more_callback' );
function cosmetsy_load_more_callback() {
