...
## Ajax Login Scripts
*************************************************/ 
function cosmetsy_ajax_login_scripts() {
	wp_enqueue_script( 'wc-password-strength-meter');
	wp_enqueue_script( 'cosmetsy-ajax-login',   plugins_url( 'js/ajax-login.js', __FILE__ ), false, '1.0');
	wp_enqueue_style( 'cosmetsy-ajax-login',    plugins_url( 'css/ajax-login.css', __FILE__ ), false, '1.0');
	wp_localize_script( 'cosmetsy-ajax-login', 'MyAjax', array(
		'ajaxurl' => esc_url(admin_url( 'admin-ajax.php' )),
	));
}
add_action( 'woocommerce_before_customer_login_form', 'cosmetsy_ajax_login_scripts' );

/*************************************************
## Ajax Login CallBack
*************************************************/ 
add_action( 'wp_ajax_nopriv_ajax_login', 'cosmetsy_ajax_login_callback' );
add_action( 'wp_ajax_ajax_login', 'cosmetsy_ajax_login_callback' );
function cosmetsy_ajax_login_callback() {
	...
	try {
		...
		if ( $validation_error->get_error_code() ) {
			throw new Exception( '<strong>' . esc_html__( 'Error:', 'cosmetsy-core' ) . '</strong> ' . $validation_error->get_error_message() );
		}

		if ( empty( $creds['user_login'] ) ) {
			throw new Exception( '<strong>' . esc_html__( 'Error:', 'cosmetsy-core' ) . '</strong> ' . esc_html__( 'Username is required.', 'cosmetsy-core' ) );
		}
		...
		if ( $e->getMessage() ) {
			echo cosmetsy_sanitize_data($e->getMessage());
			exit;
		}
	}
}
...
*************************************************/ 
add_action( 'wp_ajax_nopriv_ajax_register', 'cosmetsy_ajax_register_callback' );
add_action( 'wp_ajax_ajax_register', 'cosmetsy_ajax_register_callback' );
function cosmetsy_ajax_register_callback() {
	...
	try {
		...
		if ( 1 === count( $validation_errors ) ) {
			...
			foreach ( $validation_errors as $message ) {
				wc_add_notice( '<strong>' . esc_html__( 'Error:', 'cosmetsy-core' ) . '</strong> ' . $message, 'error' );
			}
			throw new Exception();
		}
		...
		if ( 'yes' === get_option( 'woocommerce_registration_generate_password' ) ) {
			wc_add_notice( esc_html__( 'Your account was created successfully and a password has been sent to your email address.', 'cosmetsy-core' ) );
		} else {
			wc_add_notice( esc_html__( 'Your account was created successfully. Your login details have been sent to your email address.', 'cosmetsy-core' ) );
		}
