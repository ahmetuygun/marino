...
## Ajax Search Scripts
*************************************************/ 
function cosmetsy_ajax_search_scripts() {
	wp_enqueue_style( 'cosmetsy-ajax-search',    plugins_url( 'css/ajax-search.css', __FILE__ ), false, '1.0');
	wp_enqueue_script( 'cosmetsy-ajax-search',    plugins_url( 'js/ajax-search.js', __FILE__ ), false, '1.0');
	wp_localize_script( 'cosmetsy-ajax-search', 'cosmetsysearch', array(
		'ajaxurl' => esc_url(admin_url( 'admin-ajax.php' )),
	));
}
add_action( 'wp_enqueue_scripts', 'cosmetsy_ajax_search_scripts' );


/*************************************************
## Ajax Login CallBack
*************************************************/ 
add_action( 'wp_ajax_nopriv_ajax_search', 'cosmetsy_ajax_search_callback' );
add_action( 'wp_ajax_ajax_search', 'cosmetsy_ajax_search_callback' );
function cosmetsy_ajax_search_callback() {
	global $wpdb;
	...
	if ( $args->have_posts() ) {
		...
		if($args->found_posts > 7){
			...
			echo '<a href="'.esc_url($searchall).'"><span>'.esc_html__('See all products...','cosmetsy-core').'</span> <span>('.esc_html($args->found_posts).')</span></a>';
			echo '</li>';
		}
		...
		echo '<ul><li><span>'.sprintf(esc_html__( 'No results found for "%s"', 'cosmetsy-core' ), esc_html( $keyword )).'</span></li></ul>';
	}
...
*************************************************/ 
if ( ! function_exists( 'cosmetsy_sku_search_query' ) ) {
	function cosmetsy_sku_search_query( $where, $s ) {
		global $wpdb;
...
add_filter( 'posts_search', 'cosmetsy_product_search_sku', 9 );
if ( ! function_exists( 'cosmetsy_product_search_sku' ) ) {
	function cosmetsy_product_search_sku( $where, $class = false ) {
		...
		return cosmetsy_sku_search_query( $where, $s );
